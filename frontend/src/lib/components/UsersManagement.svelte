<script lang="ts">
  ...
  async function loadData(isSearching = false) {
    try {
  ...
  $effect(() => {
    if (browser && mounted) {
      ...
      const debounce = setTimeout(() => {
        loadData(true); // Pass true to indicate this is a search operation
      }, 300);
  ...
  <div class="space-y-4 relative">
    {#if searching}
      <div class="absolute top-0 right-0 z-10">
        <div class="flex items-center gap-2 px-3 py-1 rounded-lg bg-slate-800/80 backdrop-blur-sm border border-slate-700">
