...
export class GitHubAPI {
  ...
  async createIssue(
    ...
    body: string,
    labels?: string[]
  ): Promise<{ html_url: string; number: number; id: number }> {
    logWithContext('GITHUB_API', 'Creating issue', { owner, repo, title });
    ...
    if (!response.ok) {
      ...
      logWithContext('GITHUB_API', 'Failed to create issue', { status: response.status, error });
      throw new Error(`Failed to create issue: ${response.status} ${error}`);
    }

    return response.json() as Promise<{ html_url: string; number: number; id: number }>;
  }
  ...
  async removeLabel(owner: string, repo: string, issueNumber: number, labelName: string): Promise<void> {
    logWithContext('GITHUB_API', 'Removing label from issue', { owner, repo, issueNumber, labelName });
    ...
    if (!response.ok && response.status !== 404) {
      const error = await response.text();
      logWithContext('GITHUB_API', 'Failed to remove label', { status: response.status, error });
      throw new Error(`Failed to remove label: ${response.status} ${error}`);
    }

    logWithContext('GITHUB_API', 'Label removed successfully', { labelName });
  }
}
